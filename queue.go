...
func (q *unsafeQueue) PollN(n int) []interface{} {
	var arr []interface{}
	if q.IsEmpty() {
		return arr
	}
	for i := 0; i < n; i++ {
		arr = append(arr, q.data.Remove(q.data.Front()))
	}
	return arr
}
...
func (q *unsafeQueue) PeekN(n int) []interface{} {
	var arr []interface{}
	if q.IsEmpty() {
		return arr
	}
	for i := 0; i < n; i++ {
		arr = append(arr, q.data.Front().Value)
	}
	return arr
}
...
func (q *safeQueue) PollN(n int) []interface{} {
	q.rwMutex.Lock()
	defer q.rwMutex.Unlock()
	return q.queue.PollN(n)
}
...
func (q *safeQueue) PeekN(n int) []interface{} {
	q.rwMutex.Lock()
	defer q.rwMutex.Unlock()
	return q.queue.PeekN(n)
}
